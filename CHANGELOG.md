...
- `.github/workflows/tests.yml`, a GitHub Actions workflow that runs the `pytest` suite on every PR targeting `main` (and on pushes to `main`), so PRs show a passing/failing test check before merge.
